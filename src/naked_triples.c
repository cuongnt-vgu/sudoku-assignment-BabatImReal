    ...
    for (int i =0; i < BOARD_SIZE; i++){
        if (p_cells[i]->num_candidates == 2 || p_cells[i]->num_candidates == 3){
            if((is_candidate(p_cells[i], value1) && is_candidate(p_cells[i], value2)) ||
             ...
             (is_candidate(p_cells[i], value1) && is_candidate(p_cells[i], value2) && is_candidate(p_cells[i], value3))){
            count ++;
            }
        }
    }
...
int is_naked_triple_cell(Cell *p_cell, int value1, int value2, int value3){
    if (p_cell->num_candidates == 2 || p_cell->num_candidates == 3){
        return ((is_candidate(p_cell, value1) && is_candidate(p_cell, value2)) ||
             (is_candidate(p_cell, value2) && is_candidate(p_cell, value3)) || 
             (is_candidate(p_cell, value1) && is_candidate(p_cell, value3)) ||
             (is_candidate(p_cell, value1) && is_candidate(p_cell, value2) && is_candidate(p_cell, value3))); 
    }
    return 0;
}
    ...
    for (int i = 0; i < num_value-2; i++){
        for (int j = i+1; j < num_value-1; j++){
            for (int k = j+1; k < num_value; k++){
                if(is_naked_triple_set(p_cells, naked_triple_value_array[i], naked_triple_value_array[j], naked_triple_value_array[k])){
                    for (int l = 0; l < BOARD_SIZE-2; l++){
                        for (int m = l+1; m < BOARD_SIZE-1; m++){
                            for (int n = m+1; n < BOARD_SIZE; n++){
                                if ((is_naked_triple_cell(p_cells[l], naked_triple_value_array[i], naked_triple_value_array[j], naked_triple_value_array[k])) && (is_naked_triple_cell(p_cells[m], naked_triple_value_array[i], naked_triple_value_array[j], naked_triple_value_array[k])) && (is_naked_triple_cell(p_cells[n], naked_triple_value_array[i], naked_triple_value_array[j], naked_triple_value_array[k]))) {
...
int naked_triple(SudokuBoard *p_board)
{
    ...
    for (int i = 0; i < p_counter; i++){
        ...
        if (is_in_naked_triple_list(solved_cell, p_naked_triple[i].p_cell1))
        {
            overlap += 1;
            continue;
        }
        printf("%d %d %d", p_naked_triple[i].value1, p_naked_triple[i].value2, p_naked_triple[i].value3 );
        solved_cell[solved_cell_count++] = p_naked_triple[i];

        for (int k = 0; k < BOARD_SIZE; k++)
        {
            if (p_cells[k] == p_naked_triple[i].p_cell1 || p_cells[k] == p_naked_triple[i].p_cell2 || p_cells[k] == p_naked_triple[i].p_cell2) continue;
